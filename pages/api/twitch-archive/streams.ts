...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const [queryResult]: [QueryResult, FieldPacket[]] = await conn.query(`SELECT * FROM streams ORDER BY timestamp ${sortCode}`);
    conn.end();
    const rows: Stream[] = queryResult as QueryResult & { rows: Stream[] } & Stream[];
